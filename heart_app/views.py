...
def _predict(form_data: dict) -> dict:
    ...
    if not _load_models():
        # Demo mode – return a dummy result if models not present
        import random
        pred = random.randint(0, 1)
        return {'prediction': pred, 'probability': round(random.uniform(0.4, 0.95), 2), 'demo': True}
...
def predict(request):
    ...
    result = _predict({
        ...
        'resting_bp': cd['resting_bp'], 'cholesterol': cd['cholesterol'],
        'fasting_bs': cd['fasting_bs'], 'resting_ecg': cd['resting_ecg'],
        'max_hr': cd['max_hr'], 'exercise_angina': cd['exercise_angina'],
